...
// using ThreadCar (and events) code as reference but creating a more generic post type that can support all types of posts
...
export default function Post({ post }) { 
    ...
    const eventTime = `${post.get('eventTime').toLocaleString('en-Gb', {weekday:'short'})}, ${post.get('eventTime').toLocaleString('en-Gb', {day:'numeric', month:'short'})} at ${post.get('eventTime').toLocaleString('en-US', {hour:'numeric', minute:'numeric', hour12: false})}`
    ...
    const timeSincePost = () => {
        ...
        if (timeDiff < 60) {
            ...
            value = Math.round((Date.now() - post.createdAt) / (1000*60*60*24*30))
            unit = 'month'
        }
    ...
    if (post.get('category') === 'Event') {
        return (
            ...
            <header className="event_card_header">
                <EventIcon fontSize='large' className={`event-icon ${post.get('eventCategory')}`} />
                <h1 className={`title ${post.get('eventCategory')}`}>{post.get('postTitle')}</h1>
            </header>
